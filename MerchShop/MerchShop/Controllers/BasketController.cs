    ...
    public class BasketController : Controller
    {
		...
		public string RemoveOutBasketView(int id)
		{
			...
			if (product != null)
			{
				...
				if (productCount > 0)
				{
					return $"<div class=\"button-wrapper-row\"><button onclick=\"removeOutBasket({id})\" class=\"primary-btn\">-</button><button  class=\"primary-btn\" onclick=\"addInBasket({id})\">+</button></div>";
				}
			}
			return $"<button class=\"primary-btn\"  type=\"button\" onclick=\"addInBasket({id})\">В корзину</button>\r\n <button class=\"primary-btn\">Купить сейчас</button>";
